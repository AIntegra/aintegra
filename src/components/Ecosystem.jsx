...
export default function Ecosystem({ t }) {
  return (
    <section>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {t.ecosystem.items.map((e, i) => (
          ...
          >
            <e.icon className="h-6 w-6 mb-3 text-indigo-300" />
            <h4 className="font-medium text-lg mb-1">{e.title}</h4>
            <p className="text-sm text-neutral-300 leading-relaxed">{e.body}</p>
          </motion.div>
        ))}
      </div>
    </section>
